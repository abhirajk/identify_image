...
def drawDetectedImage(frame: DetectedFrame):
    ...
    for target in frame.targets:
        ...
        cv2.putText(imageCopy, target.kind, (int(target.location.dx) + 10, int(target.location.dy) + 10), font, 1,
                    (0, 0, 255), 2, cv2.LINE_AA);
    return imageCopy;


def drawOriginalImage(frame: DetectedFrame):
    ...
    for target in frame.targets:
        ...
        cv2.rectangle(imageCopy, rect_start, rect_end, (0, 255, 0, 0))
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(imageCopy, target.kind, (int(target.location.x) + 10, int(target.location.y) + 10), font, 1,
                    (0, 0, 255), 2, cv2.LINE_AA);
    return imageCopy;
